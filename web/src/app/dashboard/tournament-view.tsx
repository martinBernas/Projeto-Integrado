import { formatDate, formatScore, ranking, type TournamentData } from '@/lib/tournament';

export function TournamentView({ data, userId }: { data: TournamentData; userId: string }) {
  const standings = ranking(data.participants, data.results);
  const days = [...new Set(data.results.map(r => r.played_on))].sort().reverse();
  return <>
    ...
    <section className="space-y-3"><h2 className="text-xl font-bold">Resultados por dia</h2>
      ...
      {days.map(day => {
        const rows = data.results.filter(r => r.played_on === day);
